...
## Contas de utilizador (email + palavra-passe)

O site agora tem um sistema simples de contas (`/entrar` e `/criar-conta`),
sem Google nem outros fornecedores externos — só email e palavra-passe. É
construído com **NextAuth (Auth.js)** + **Vercel Postgres**, ambos serviços
nativos da Vercel.

Para funcionar em produção, depois do deploy:

1. No dashboard da Vercel, abre o projeto → separador **Storage** → **Create
   Database** → escolhe **Postgres** → liga-a a este projeto. A Vercel injeta
   sozinha a variável `POSTGRES_URL` (e outras) — não precisas de copiar nada.
2. Ainda em **Settings → Environment Variables**, adiciona:
   - `NEXTAUTH_SECRET` — uma string aleatória. Podes gerar uma correndo
     `openssl rand -base64 32` no terminal.
3. Volta a fazer deploy (ou espera pelo próximo deploy automático) para que
   as novas variáveis fiquem disponíveis.

A tabela `users` é criada automaticamente na base de dados na primeira vez
que alguém cria conta — não precisas de correr nenhum SQL manualmente.

Para correr localmente, copia `.env.example` para `.env.local` e preenche os
valores (podes usar a mesma `POSTGRES_URL` da Vercel, disponível em
Storage → a tua base de dados → `.env.local` tab).
